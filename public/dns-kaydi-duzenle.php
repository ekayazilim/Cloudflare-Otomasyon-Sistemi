...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($ad)) {
        ...
        $dns_guncelle_sonuc = $cloudflare->dnsKaydiGuncelle($domain_bilgi['zone_id'], $dns_kaydi['record_id'], $dns_veri);
        
        if (isset($dns_guncelle_sonuc['success']) && $dns_guncelle_sonuc['success']) {
            // Veritabanındaki DNS kaydını güncelle
            $db_dns_veri = [
                'isim' => $ad,
                'tip' => $tip,
                'icerik' => $icerik,
            ...
            // MX kaydı için öncelik ekle
            if ($tip === 'MX') {
                $db_dns_veri['oncelik'] = (int)$oncelik;
            }
            
            // Proxy durumunu ekle
            $db_dns_veri['proxied'] = $proxy;
            
            $guncellendi = $domain->dnsKaydiGuncelle($kayit_id, $db_dns_veri);
            
            if ($guncellendi) {
                $basari_mesaji = 'DNS kaydı başarıyla güncellendi.';
                
                // Yönlendirme için session mesajı ayarla
                $_SESSION['mesaj'] = 'DNS kaydı başarıyla güncellendi.';
                ...
                $hata_mesaji = 'DNS kaydı Cloudflare\'de güncellendi ancak veritabanında güncellenirken bir hata oluştu.';
            }
        } else {
            $hata_mesaji = 'DNS kaydı güncellenirken bir hata oluştu: ' . 
                (isset($dns_guncelle_sonuc['errors']) && is_array($dns_guncelle_sonuc['errors']) && isset($dns_guncelle_sonuc['errors'][0]['message']) ? $dns_guncelle_sonuc['errors'][0]['message'] : 'Bilinmeyen hata');
        }
    }
}
...
<div class="row">
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1>DNS Kaydı Düzenle: <?php echo $domain_bilgi['domain']; ?></h1>
...
<div class="card">
    <div class="card-header bg-warning text-dark">
        <h5 class="mb-0">DNS Kaydı Düzenle</h5>
    </div>
    <div class="card-body">
        <form method="post" action="">
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        <?php
                        // İsim alanını kontrol et (ad veya isim olarak gelebilir)
                        $kayit_adi = isset($dns_kaydi['isim']) ? $dns_kaydi['isim'] : (isset($dns_kaydi['ad']) ? $dns_kaydi['ad'] : '');
                        $alan_adi = str_replace('.' . $domain_bilgi['domain'], '', $kayit_adi);
                        ?>
                        <input type="text" class="form-control" id="dns_ad" name="dns_ad" value="<?php echo htmlspecialchars($alan_adi); ?>" required>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="dns_tip" name="dns_tip" required>
                        ...
                        <option value="A" <?php echo $dns_kaydi['tip'] == 'A' ? 'selected' : ''; ?>>A (IPv4 Adresi)</option>
                        <option value="AAAA" <?php echo $dns_kaydi['tip'] == 'AAAA' ? 'selected' : ''; ?>>AAAA (IPv6 Adresi)</option>
                        <option value="CNAME" <?php echo $dns_kaydi['tip'] == 'CNAME' ? 'selected' : ''; ?>>CNAME (Canonical Name)</option>
                        <option value="MX" <?php echo $dns_kaydi['tip'] == 'MX' ? 'selected' : ''; ?>>MX (Mail Exchange)</option>
                        <option value="TXT" <?php echo $dns_kaydi['tip'] == 'TXT' ? 'selected' : ''; ?>>TXT (Text)</option>
                        <option value="NS" <?php echo $dns_kaydi['tip'] == 'NS' ? 'selected' : ''; ?>>NS (Name Server)</option>
                        <option value="SRV" <?php echo $dns_kaydi['tip'] == 'SRV' ? 'selected' : ''; ?>>SRV (Service)</option>
                        <option value="CAA" <?php echo $dns_kaydi['tip'] == 'CAA' ? 'selected' : ''; ?>>CAA (Certificate Authority Authorization)</option>
                    </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="dns_icerik" name="dns_icerik" value="<?php echo htmlspecialchars($dns_kaydi['icerik']); ?>" required>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="dns_ttl" name="dns_ttl" required>
                        <option value="1" <?php echo $dns_kaydi['ttl'] == 1 ? 'selected' : ''; ?>>Otomatik</option>
                        <option value="60" <?php echo $dns_kaydi['ttl'] == 60 ? 'selected' : ''; ?>>1 dakika</option>
                        <option value="300" <?php echo $dns_kaydi['ttl'] == 300 ? 'selected' : ''; ?>>5 dakika</option>
                        <option value="600" <?php echo $dns_kaydi['ttl'] == 600 ? 'selected' : ''; ?>>10 dakika</option>
                        <option value="1800" <?php echo $dns_kaydi['ttl'] == 1800 ? 'selected' : ''; ?>>30 dakika</option>
                        <option value="3600" <?php echo $dns_kaydi['ttl'] == 3600 ? 'selected' : ''; ?>>1 saat</option>
                        <option value="7200" <?php echo $dns_kaydi['ttl'] == 7200 ? 'selected' : ''; ?>>2 saat</option>
                        <option value="18000" <?php echo $dns_kaydi['ttl'] == 18000 ? 'selected' : ''; ?>>5 saat</option>
                        <option value="43200" <?php echo $dns_kaydi['ttl'] == 43200 ? 'selected' : ''; ?>>12 saat</option>
                        <option value="86400" <?php echo $dns_kaydi['ttl'] == 86400 ? 'selected' : ''; ?>>1 gün</option>
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" class="form-control" id="dns_oncelik" name="dns_oncelik" value="<?php echo isset($dns_kaydi['oncelik']) ? (int)$dns_kaydi['oncelik'] : 10; ?>" min="0" max="65535">
                    <div class="form-text">MX kayıtları için öncelik değeri. Düşük değerler daha yüksek önceliğe sahiptir.</div>
                </div>
            </div>
            
            <div class="mb-3 form-check">
                <input type="checkbox" class="form-check-input" id="dns_proxy" name="dns_proxy" value="1" <?php echo isset($dns_kaydi['proxy']) && $dns_kaydi['proxy'] == 1 ? 'checked' : ''; ?>>
            ...
            <div class="d-grid gap-2">
                <button type="submit" class="btn btn-warning">
                    <i class="fas fa-save"></i> DNS Kaydını Güncelle
                </button>
                <a href="dns-kayitlari.php?domain_id=<?php echo $dns_kaydi['domain_id']; ?>" class="btn btn-secondary">
